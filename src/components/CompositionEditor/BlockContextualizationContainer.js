...
class BlockContainer extends Component {
  ...
  render() {
    ...
    const setOptionImageDrop = files => {
      const file = files[0];
      fileIsAnImage(file)
        .then(thatFile => {
            return thatFile;
    ...
    const setPreviewMode = thatPreviewMode => {
      onAssetChange('contextualizer', contextualizer.id, {
        ...contextualizer,
        previewMode: thatPreviewMode
      });
    };

    const setHideInCodexMode = hideInCodexMode => {
      if (contextualizer.visibility) {
        onAssetChange('contextualizer', contextualizer.id, {
          ...contextualizer,
          visibility: {
            ...contextualizer.visibility,
            codex: !hideInCodexMode
          },
        });
      }
      else {
    ...
    const setHideInWebMode = hideInWebMode => {
      if (contextualizer.visibility) {
        onAssetChange('contextualizer', contextualizer.id, {
          ...contextualizer,
          visibility: {
